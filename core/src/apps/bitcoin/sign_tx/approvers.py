...
class Approver:
    ...
    async def add_external_output(
        self, txo: TxOutputType, script_pubkey: bytes, orig_txo: Optional[TxOutputBinType]
    ) -> None:
        self.weight.add_output(script_pubkey)
        self.total_out += txo.amount

    async def add_removed_orig_external_output(
        self, txo: TxOutputBinType
    ) -> None:
        self.orig_total_out += txo.amount
...
class BasicApprover(Approver):
    ...
    async def add_external_output(
        ...
        # TODO if orig_txo is set, then confirm differences in amount
